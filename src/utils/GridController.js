class GridController {
  countColumns(grid) {
    const columnsNumber = window.getComputedStyle(grid).getPropertyValue('grid-template-columns').split(' ').length;
    return columnsNumber;
  }

  countRows(grid) {
    const rowsNumber = window.getComputedStyle(grid).getPropertyValue('grid-template-rows').split(' ').length;
    return rowsNumber;
  }
  ...
  slideGrid(grid, direction) {
    ...
    if (direction === 'right') {
      ...
      if (newRowIndex.at(-1) > gridItems.length - 1) {
        gridItems.slice(-colsPerRow).forEach((item) => (item.style.display = 'block'));
      ...
      gridItems.slice(newRowIndex.at(0), newRowIndex.at(-1) + 1).forEach((item) => (item.style.display = 'block'));
    ...
    if (newRowIndex.at(0) < 0 && newRowIndex.at(-1) + 1 >= 0) {
      if (newRowIndex.at(-1) + 1 === 0) {
        return gridItems.slice(newRowIndex.at(0)).forEach((item) => (item.style.display = 'block'));
      }
    ...
    return gridItems.slice(newRowIndex.at(0), newRowIndex.at(-1) + 1).forEach((item) => (item.style.display = 'block'));
  }
}
